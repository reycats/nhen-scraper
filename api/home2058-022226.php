<?php
/**
 * ORIENTAL SEARCH v6.5 - MASTER ARATA EDITION [cite: 2026-02-22]
...
<body class="bg-dark">
    ...
    <div class="genre-popup" id="genrePopup">
        ...
        <form id="multiSearchForm" action="home.php" method="GET">
            ...
            <div class="text-center"><?php foreach($cat_characters as $g): ?>
                <input type="checkbox" name="tags[]" value="<?= $g ?>" id="t_<?= md5($g) ?>" class="multi-tag">
                <label for="t_<?= md5($g) ?>" class="tag-label"><?= strtoupper($g) ?></label>
            <?php endforeach; ?></div>
            <div class="cat-title">🧧 KUALITAS VISUAL</div>
            <div class="text-center"><?php foreach($cat_visual as $g): ?>
                <input type="checkbox" name="tags[]" value="<?= $g ?>" id="t_<?= md5($g) ?>" class="multi-tag">
                <label for="t_<?= md5($g) ?>" class="tag-label"><?= strtoupper($g) ?></label>
            <?php endforeach; ?></div>
            <div class="sticky-bottom bg-dark p-3 mt-4 border-top border-warning text-center">
                <input type="hidden" name="q" id="finalQuery">
                <button type="button" class="btn btn-warning fw-bold px-5 py-3 shadow" onclick="submitMulti()">🚀 GAS CULIK KOMBINASI</button>
            </div>
        </form>
    </div>

<?php
/**
 * ORIENTAL READER v4.0 - ANTI-HITAM SAKTI [cite: 2026-02-22]
 * Fix Masalah Rekomendasi Gak Bawa GID bbi!
 */
require_once 'config.php';
$id = $_GET['id'] ?? ''; 
$gid = $_GET['gid'] ?? ''; // Masalahnya di sini asu, sering kosong dari recom!
$pages = $_GET['pages'] ?? 35;

if (!$id) die("ID nuklir gaib asu!");

// ===== SAKTI GID REPAIR [cite: 2026-02-22] =====
// Kalau GID kosong, sama kayak ID, atau dongo (kependekan), paksa scrape bbi!
if (empty($gid) || $gid == $id || strlen($gid) < 5) {
    $apiUrl = "https://nhentai.net/api/gallery/" . $id;
    $ch = curl_init($apiUrl);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36');
    $res = json_decode(curl_exec($ch), true); curl_close($ch);
    
    // Culik Media ID yang beneran asu! [cite: 2026-02-22]
    $gid = $res['media_id'] ?? $id; 
}
...
<head>
    ...
    <style>
        ...
        .navbar { background: #8B0000; position: fixed; top: 0; width: 100%; height: 60px; display: flex; align-items: center; justify-content: space-between; padding: 0 20px; z-index: 9999; border-bottom: 2px solid #D4AF37; transition: transform 0.3s; }
        ...
        .recom-section { margin-top: 50px; padding: 30px; border-top: 3px solid #D4AF37; background: #111; }
...
<body>
    ...
    <div class="reader-container">
        ...
        <div class="recom-section">
            <h4 class="text-warning mb-4">🏮 REKOMENDASI ELIT 🏮</h4>
            <div class="recom-grid">
                <?php foreach($recoms as $r): ?>
                    <div class="recom-item">
                        <a href="reader.php?id=<?= $r[1] ?>&pages=35" style="text-decoration:none;">
                            <img src="<?= $r[2] ?>">
                            <div style="font-size:0.75rem; color:#FFD700; height:35px; overflow:hidden; margin-top:8px;"><?= htmlspecialchars($r[3]) ?></div>
                        </a>
                    </div>
                <?php endforeach; ?>
            </div>
    ...
    <script>
        var lastScroll = 0; window.onscroll = function() {
            var st = window.pageYOffset || document.documentElement.scrollTop;
            var header = document.getElementById("header");
            if (st > lastScroll && st > 100) { header.classList.add("nav-up"); } else { header.classList.remove("nav-up"); }
